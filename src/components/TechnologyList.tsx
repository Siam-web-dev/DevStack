...
const TechnologyList = ( {
    ...
    return (
        <div className=" p-4 h-fit w-full">
            ...
            <p className="text-[#64748B] text-sm mt-3 mb-3">{technology.length} Technology Selected</p>

            <div className="space-y-2">
                {technology.length === 0 ? (
                    <p className="text-sm text-[#64748B] "> No technology selected</p>
                ) : (
                    technology.map((item) => (
                        ...
                        <button onClick={ () => removeStack(item.id)} >
        ...
        <button
          onClick={removeAll}
          className="mt-4 w-full text-red-500 border border-red-500 rounded-lg py-2 text-sm"
        >
          Remove All
        </button>
      )}
